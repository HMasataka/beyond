...
台帳の書式と運用は `docs/learnings/README.md`、設計判断は `docs/adr/0010-review-learnings.md` と `docs/adr/0011-review-learnings-auto-extraction.md` を参照する。
...
5. レビュー指摘パターンの知見だけを判断し、`capture.sh --origin session --id <session-id> --date <date>` で記録する。会話由来は PR 番号が無いため `origin=session` で記録する。
...
ノイズ対策（承認待ち・定期 prune）と外部レビュー連携は未実装。設計は `docs/adr/0010-review-learnings.md` と `docs/adr/0011-review-learnings-auto-extraction.md` を参照する。
