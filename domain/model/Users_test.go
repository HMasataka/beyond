...
func testUserToManyUserCards(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.UserCards().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserCards(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserCards); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserCards = nil
	if err = a.L.LoadUserCards(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserCards); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUserCards(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserCards(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserCards[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserCards[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserCards().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
