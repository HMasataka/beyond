...
- 残る制約: 抽出の質は本体エージェントの判断に依存する。ノイズ対策（承認待ち・定期 prune）は本 ADR の対象外とし、`docs/adr/0012-review-learnings-noise-control.md` で扱う。codex 環境が手元に無いため、codex の `transcript_path` の実体と append-only 性は実機未検証で、degrade 時は会話抽出のみスキップされる。backfill の対象は保留キューに記録された未抽出 session に限り、過去全件は遡及しない。
- 却下した代替案:
