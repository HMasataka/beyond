...
この ADR では手動 capture と卒業提案フックまでを対象とし、会話ログからの自動抽出は別の決定（`docs/adr/0011-review-learnings-auto-extraction.md`）で扱う。
...
- `status` の遷移（`promoted` / `retired`）は人の手動編集のみ。自動遷移しない。
...
- 残る制約: 観測の記録は手動 capture に依存する。記録を怠れば台帳は育たない。会話ログからの自動抽出は別の決定（`docs/adr/0011-review-learnings-auto-extraction.md`）に委ねる。卒業先の選択と status 遷移は人が担うため、運用の規律が要る。
- 却下した代替案:
