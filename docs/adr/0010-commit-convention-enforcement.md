# 0010. コミットメッセージと PR タイトルを commitlint で強制する
...
- 残る制約: ルートに Node の依存と `node_modules` が増える。フックはローカルの `nix develop` 環境に husky と pnpm がある前提で動く。
- 却下した代替案: commitlint を `web/package.json` に同居させる案は、CI の pnpm 基盤を流用できる利点はあるが、web 固有でない関心事を web に持ち込むため採らない。commitlint を nix flake の devShell で供給する案は、ツール供給方針には沿うが、`@commitlint/config-conventional` の解決に追加の作業が要るため採らない。
